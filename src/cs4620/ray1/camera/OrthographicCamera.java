...
public class OrthographicCamera extends Camera {
    ...
    public void initView() {
    	...
    	basisW.normalize();
    	basisU.set(viewUp.clone().cross(basisW).normalize());
    ...
    public void getRay(Ray outRay, double inU, double inV) {
    	...
    	outRay.origin.set(basisU.clone().mul(inU).addMultiple(inV, basisV).add(viewPoint));
    	outRay.direction.setMultiple(-1,basisW);
