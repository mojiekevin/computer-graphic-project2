...
public class Scene {
	...
	private boolean intersect(IntersectionRecord outRecord, Ray rayIn, boolean anyIntersection) {
		...
		IntersectionRecord outRecordcopy = new IntersectionRecord();
		outRecordcopy.set(outRecord);
		for (int i = 0; i < surfaces.size(); i++){
			if(surfaces.get(i).intersect(outRecord, rayIn)){
				if(anyIntersection){
					outRecord.set(outRecordcopy);
					return true;
				}
				if(outRecord.t > rayIn.start && outRecord.t < rayIn.end){
					rayIn.end = outRecord.t;
				}
			}
		}
		for (int j = 0; j < surfaces.size(); j++){
			if(surfaces.get(j).intersect(outRecord, rayIn)){
